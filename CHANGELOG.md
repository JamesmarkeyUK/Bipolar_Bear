...
## v1.14
- 🐛 Fix (medication data loss): medications entered during the tutorial as a guest were saved only to `localStorage` (`currentMedList`). Creating an account never uploaded that local data to Firestore — the auth listeners only ever pulled settings *down* from the account — so a user's meds were never backed up and disappeared the moment `localStorage` was cleared (app reinstall, storage eviction, switching device/web↔native). The auth listeners on the home, journal, and survival-kit pages now back guest-entered meds *up* to `userSettings/{uid}` on sign-in/sign-up whenever the account doesn't already have a medication list, so guest meds are claimed into the new account and survive across devices. Idempotent and non-destructive (it only ever adds missing data, never overwrites an existing account list). Touches `js/index.js`, `js/journal.js`, `js/survival-kit.js`, `service-worker.js` (`CACHE_NAME` v105)
