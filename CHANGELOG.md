...
- 🔗 The **Bipolar Anonymous Google Play badges** on both landing pages now link to the listing (`com.bipolaranonymous.app`) instead of sitting inert on `href="#"` — four of them: the Anonymous showcase on `welcome.html`, the hero and showcase on `welcome-anonymous.html`, and the text link in its footer. The four App Store badges are still placeholders, waiting on the listing's numeric Apple ID. Touches `welcome.html`, `welcome-anonymous.html` (`CACHE_NAME` v215)
